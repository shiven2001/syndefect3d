...
def static_category_factory(
    ...
    class StaticCategoryFactory(StaticAssetFactory):
        ...
        def spawn_asset(self, i, **kwargs):
            ...
            # Extract asset filename without extension to use as suffix
            asset_name = os.path.splitext(self.asset_file)[0]
            
            # Append asset name to the object name
            if obj.name.endswith(")"):
                # Format: StaticCategoryFactory_813587__spawn_asset(5167484)
                obj.name = f"{obj.name[:-1]}_{asset_name})"
            else:
                # Fallback: just append the asset name
                obj.name = f"{obj.name}_{asset_name}"
