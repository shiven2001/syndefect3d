...
def apply_baked_tex(obj, paramDict={}):
    ...
    for slot in obj.material_slots:
        ...
        # Before deleting nodes, preserve alpha connection information
        ...
        if nodes.get("Principled BSDF"):
            ...
            if alpha_input and alpha_input.is_linked:
                ...
                if from_node.bl_idname == "ShaderNodeRGBToBW":
                    ...
                    if from_node.inputs["Color"].is_linked:
                        ...
                        if mask_tex.type == "TEX_IMAGE" and mask_tex.image:
                            ...
                            logging.info(f"Found mask texture via RGBToBW: {mask_tex.name}")
                    ...
                    if "mask" in node_label or "mask" in node_name:
                        ...
                        if "opacity" in node_label or "alpha" in node_label:
                            ...
                            logging.info(f"Found opacity texture for alpha: {from_node.name}")
                    else:
                        ...
                        img_name = from_node.image.name.lower() if from_node.image.name else ""
                        if "mask" in img_name:
                            alpha_connection_info = {
                                "type": "mask_texture",
                                "image": from_node.image,
                            ...
                            logging.info(f"Found mask texture (by image name) for alpha: {from_node.name}")
        ...
        for type in ALL_BAKE:
            ...
            if type == "NORMAL":
                ...
                continue
            # For Base Color, only connect Color output, not Alpha
            if type == "DIFFUSE" and alpha_connection_info:
                # Don't let the diffuse/albedo texture's alpha override the mask
                links.new(principled_bsdf_node.inputs[ALL_BAKE[type]], tex_node.outputs["Color"])
            else:
                links.new(principled_bsdf_node.inputs[ALL_BAKE[type]], tex_node.outputs[0])

        # Restore alpha connection if we found one
        if alpha_connection_info and alpha_connection_info["image"]:
            alpha_img = alpha_connection_info["image"]
            
            # Create a new image texture node for the mask/opacity
            alpha_tex_node = nodes.new("ShaderNodeTexImage")
            alpha_tex_node.image = alpha_img
            alpha_tex_node.label = "Mask/Opacity"
            
            if alpha_connection_info["type"] == "mask_texture_rgb2bw":
                # For mask textures, convert to grayscale first
                rgb2bw = nodes.new("ShaderNodeRGBToBW")
                links.new(alpha_tex_node.outputs["Color"], rgb2bw.inputs["Color"])
                links.new(rgb2bw.outputs["Val"], principled_bsdf_node.inputs["Alpha"])
                logging.info("Reconnected mask texture to Alpha via RGBToBW")
            elif alpha_connection_info.get("use_alpha_channel"):
                # For opacity textures, use the Alpha output
                links.new(alpha_tex_node.outputs["Alpha"], principled_bsdf_node.inputs["Alpha"])
                logging.info("Reconnected opacity texture Alpha channel to Alpha input")
            else:
                # Direct mask texture connection
                rgb2bw = nodes.new("ShaderNodeRGBToBW")
                links.new(alpha_tex_node.outputs["Color"], rgb2bw.inputs["Color"])
                links.new(rgb2bw.outputs["Val"], principled_bsdf_node.inputs["Alpha"])
                logging.info("Reconnected mask texture to Alpha")
