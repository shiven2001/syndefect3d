...
def main(args):
    ...
    # If render task is specified, iterate over all camera rigs
    if "render" in args.task:
        # Get camera rigs from the scene
        if args.input_folder is not None:
            # Load the scene to get camera rigs
            bpy.ops.wm.open_mainfile(filepath=str(args.input_folder / "scene.blend"))
            camera_rigs = cam_util.get_camera_rigs()

            # Iterate over all camera rigs and subcameras
            for camrig_id in range(len(camera_rigs)):
                for subcam_id in range(len(camera_rigs[camrig_id].children)):
                    logger.info(
                        f"Rendering camera rig {camrig_id}, subcamera {subcam_id}"
                    )

                    # Create output folder for this camera
                    camera_output_folder = (
                        args.output_folder / f"camera_{camrig_id}_{subcam_id}"
                    )
                    camera_output_folder.mkdir(parents=True, exist_ok=True)

                    # Call execute_tasks.main for this specific camera
                    execute_tasks.main(
                        ...
                        camera_id=[camrig_id, subcam_id],
                        frame_range=[1, 1],  # Default frame range
                    )
        else:
            logger.error(
                "Input folder is required for render task to iterate over camera rigs"
            )
            return
    else:
        # For non-render tasks, use the original behavior
        execute_tasks.main(
            compose_scene_func=compose_indoors,
            populate_scene_func=None,
            input_folder=args.input_folder,
