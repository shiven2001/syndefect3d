...
def home_furniture_constraints(
    defect_count_min: int = 1,
    defect_count_max: int = 1,
):
    ...
    Args:
        defect_count_min: Minimum number of defects per room (default: 0)
        defect_count_max: Maximum number of defects per room (default: 10)
    ...
    constraints["defects"] = rooms.all(
        lambda r: (
            defect_planes.related_to(r)
            .count()
            .in_range(defect_count_min, defect_count_max)
            * defects.related_to(r).all(lambda t: (t.distance(r, cu.floortags) > 0.6))
            * defects.all(
                lambda t: (
                    (vertical_diff(t, r).abs() < 1.5) * (t.distance(cutters) > 0.1)
                )
    ...
    # Defect planes score term
    score_terms["defects"] = rooms.mean(
        lambda r: (
            defects.related_to(r).mean(
                lambda d: (
                    vertical_diff(d, r).abs().minimize(weight=1)
                    + d.distance(defects).maximize(weight=1)
