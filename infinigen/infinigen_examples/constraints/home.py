...
def home_furniture_constraints():
    ...
    # Defect planes constraint - separate from wall decorations
    # At least 1 on walls (1-10 range) and up to 5 on ceiling (0-5 range)
    constraints["defects"] = rooms.all(
        lambda r: (
            defects_wall.related_to(r)
            .count()
            .in_range(1, 10)  # Wall defects: 1-10 per room
    ...
    # Defect planes score term - optimize placement for both wall and ceiling defects
    score_terms["defects"] = rooms.mean(
        lambda r: (
            # Wall defects optimization
            defects_wall.related_to(r).mean(
                lambda d: (
                    vertical_diff(d, r).abs().minimize(weight=1)
                    + d.distance(defects_wall).maximize(weight=1)
            ...
            # Ceiling defects optimization
            + defects_ceiling.related_to(r).mean(
                lambda d: (
                    d.distance(defects_ceiling).maximize(
                        weight=1
                    )  # Spread ceiling defects apart
                    + d.distance(window).hinge(0.25, 10).maximize(weight=1)
                    + cl.accessibility_cost(d, furniture, dist=1).minimize(weight=5)
                    + cl.center_stable_surface_dist(d).minimize(weight=1)
                )
